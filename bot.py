...
bot = commands.Bot(command_prefix="+",
                   description='A bot for the House of Brilliance')

bot.starttime = datetime.datetime.now()
bot.startup_done = False

initial_extensions = ['announcements', 'mannounce', 'Reload', 'serversuggest', 'polling']

@bot.event
async def on_command_error(ctx: commands.Context, error):
    BOT_LOG_CHANNEL = bot.get_channel(561656102492831764)
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Activity(name='the brilliant suggestions!', type=discord.ActivityType.watching))
